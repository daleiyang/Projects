...
class TextCNN(object):
    ...
    def cnn(self):
        ...
        with tf.name_scope("cnn"):
            ...
            filter_shape = [self.config.kernel_size, self.config.embedding_dim, 1, self.config.num_filters]
            embedding_inputs = tf.expand_dims(embedding_inputs, -1)
	        #print('emb' + str(embedding_inputs.shape))
            ...
            #[Batch_Size, Sequence_Length - Kernel_Size + 1, , channel = 1, num_filters]
            # global max pooling layer
	        #print(conv.shape)
            gmp = tf.reduce_max(conv, reduction_indices=[1, 2], name='gmp')
            #[Batch_Size, num_filters]
	        #print('gmp' + str(gmp.shape))
        ...
        with tf.name_scope("score"):
	        ...
	        #print('fc2' + str(fc.shape))
            fc = tf.nn.relu(fc)
	        #print('fc3' + str(fc.shape))
            #[Batch_Size, config.hidden_dim]

            # 分类器
            self.logits = tf.layers.dense(fc, self.config.num_classes, name='fc2')
	        #print('logits' + str(self.logits.shape))
            #[Batch_Size, config.num_classes]
            self.y_pred_cls = tf.argmax(tf.nn.softmax(self.logits), 1)  # 预测类别
	        #print('y_pred_cls' + str(self.y_pred_cls.shape))
            #[Batch_Size]

            self.y_prob = tf.nn.softmax(self.logits)  # 预测每个类别的概率
            #[Batch_Size, config.num_classes]
